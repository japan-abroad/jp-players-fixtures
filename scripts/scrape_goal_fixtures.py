...
_FUZZY_EXCLUDE_NAMES = {
    "リールセケンペンゾーネン",
    # 2026-09-09発覚: スコットランドの"レンジャーズ"(Rangers FC)が、
    # イングランド"クイーンズ・パーク・レンジャーズ"(QPR)の後方一致に
    # 誤ってヒットしていた。area="イギリス"はイングランド/スコットランド
    # 両方を許容するエイリアスのため国名の突き合わせでは防げず、
    # 斉藤光毅(QPR)の試合日程にレンジャーズが出場する無関係な
    # スコットランドの試合が大量に紛れ込んでいた。
    "レンジャーズ",
}
